...
実測で取得の空きは**最長 14.2 分**であり、生存信号の想定間隔（6 時間 = 360 分）は
その **約 25 倍**なので、**生存信号は Doze の維持時間帯に乗る** —— 眠ったまま丸 1 日
信号が出ない、という経路は無い。

> **「2 桁小さい」ではない**（ST02 の review/code.md の R38 / I12）。
> 360 ÷ 14.2 ≒ **25 倍**で、1.4 桁にあたる。1 日 24 回という実測から維持窓の間隔を読むと
> 約 1 時間で、6 時間との比は 6 倍。**どちらの読みでも 2 桁にならない。**
> 結論（生存信号は維持時間帯に乗る）は 25 倍でも 6 倍でも成り立つので振る舞いは動かないが、
> **根拠の数は直す** —— design D10 が WCAG の 729:1 を実際に計算して結論を覆したのと同じ規律。
> 同じ誤りが `openspec/changes/st02-collection-coverage/specs/device-collection/spec.md` の
> 根拠ブロックにもある（Requirement と Scenario は動かないので、そちらは文言だけ直した）。
