...
R1=$(curl -sf -X POST "http://$BIND/ingest" -H 'content-type: application/json' -d "$BODY")
echo "   → $R1"
echo "$R1" | grep -q '"duplicate":false'

echo "== 5. もう一度同じものを送る（FR-22: 再送しても行が増えない）"
R2=$(curl -sf -X POST "http://$BIND/ingest" -H 'content-type: application/json' -d "$BODY")
echo "   → $R2"
echo "$R2" | grep -q '"duplicate":true'

echo "== 6. 取り出す（論理削除を効かせたビュー越し）"
N=$(curl -sf "http://$BIND/events" | grep -o '"id"' | wc -l)
echo "   → $N 件"
