...
pub struct ReadVisit { pub id: i64, pub url: String, pub title: Option<String>, pub at: chrono::DateTime<chrono::Utc>, pub transition: i64, pub from_visit: Option<i64>, pub duration_us: Option<i64> }
...
pub fn read_chromium(path: &std::path::Path) -> anyhow::Result<Vec<ReadVisit>> {
    ...
    let mut stmt = conn.prepare("SELECT v.id,u.url,u.title,v.visit_time,v.transition,v.from_visit,v.visit_duration FROM visits v JOIN urls u ON u.id=v.url ORDER BY v.id")?;
    let visits = stmt.query_map([], |r| Ok(ReadVisit { id:r.get(0)?, url:r.get(1)?, title:r.get(2)?, at: chromium_micros(r.get(3)?).ok_or(rusqlite::Error::InvalidQuery)?, transition:r.get(4)?, from_visit: nonzero(r.get(5)?), duration_us: Some(r.get(6)?) }))?.collect::<Result<Vec<_>, _>>()?;
    Ok(visits)
}
...
pub fn read_firefox(path: &std::path::Path) -> anyhow::Result<Vec<ReadVisit>> {
    ...
    let visits = stmt.query_map([], |r| Ok(ReadVisit { id:r.get(0)?, url:r.get(1)?, title:r.get(2)?, at: firefox_micros(r.get(3)?).ok_or(rusqlite::Error::InvalidQuery)?, transition:r.get(4)?, from_visit: nonzero(r.get(5)?), duration_us: None }))?.collect::<Result<Vec<_>, _>>()?;
    Ok(visits)
}
...
mod tests {
    ...
    fn history_read_chromium() {
        ...
        conn.execute_batch("CREATE TABLE urls(id INTEGER PRIMARY KEY, url TEXT, title TEXT); CREATE TABLE visits(id INTEGER PRIMARY KEY, url INTEGER, visit_time INTEGER, transition INTEGER, from_visit INTEGER, visit_duration INTEGER);
