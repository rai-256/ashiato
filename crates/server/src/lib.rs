...
/// 記録を 1 件受け取る。同じ内容を再送しても行は増えない（FR-22）。
#[utoipa::path(post, path = "/ingest", request_body = IngestRequest,
    responses((status = 200, body = IngestReply), (status = 400), (status = 401)))]
pub async fn ingest(
    State(app): State<App>,
    headers: HeaderMap,
...
#[openapi(
    paths(ingest, events),
    components(schemas(IngestReply, EventRow, ingest::IngestRequest)),
    info(
        title = "ashiato S-01",
        version = "0.1.0",
