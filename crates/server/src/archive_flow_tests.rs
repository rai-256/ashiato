...
/// PROBE（レビュー用。実装は触っていない）:
/// spec `印を置くと確認待ちの書庫が格納される` / `印を置いた後の読み直しは台帳に 1 行足す`
#[tokio::test]
async fn zz_probe_confirm_after_mixed_archive_never_ingests() {
    let inbox = Inbox::new("zz-probe-mixed").await;
    let watch_body = watch("2026-09-12T03:00:00Z", "ある動画");
    let unknown = myactivity("Discover");
    inbox
        .confirm(
    ...
    inbox
        ...
        .await;
    inbox
        .until("台帳に read が付かない", || async {
            inbox.ledger_rows("read").await == 1
        })
        .await;
    tokio::time::sleep(std::time::Duration::from_secs(3)).await;

    let moved = inbox.inbox.join("取り込み済み");
    println!(
        "【1】書庫は専用フォルダから消えたか: 元の場所={} / 取り込み済み={:?}",
        inbox
            .inbox
            .join("takeout-20260912T000000Z-001.zip")
            .exists(),
    ...
    // 本人が tools/archive-shape.sh --confirm で印を置く
    let shape: serde_json::Value = sqlx::query_scalar(
        "SELECT shape FROM core.archive_pending_shape WHERE user_id = $1",
    )
    .bind(inbox.user)
    .fetch_one(&inbox.pool)
    .await
    ...
    sqlx::query(
        "INSERT INTO core.archive_shape_confirmation (user_id, shape_hash, shape)
         SELECT user_id, shape_hash, shape FROM core.archive_pending_shape WHERE user_id = $1
    ...
    .await
    .unwrap();
    println!("【2】印を置いた: {shape}");

    // 走査を何周も回す
    tokio::time::sleep(std::time::Duration::from_secs(6)).await;

    let stored: i64 = sqlx::query_scalar(
        "SELECT count(*) FROM core.event WHERE user_id = $1 AND logical_source LIKE 'c03-myactivity-%'",
    )
    .bind(inbox.user)
    .fetch_one(&inbox.pool)
    .await
    ...
    let pending: i64 =
        sqlx::query_scalar("SELECT count(*) FROM core.archive_pending_shape WHERE user_id = $1")
            ...
            .await
            .unwrap();
    let reads = inbox.ledger_rows("read").await;
    println!("【3】印を置いて 6 秒（走査 6 周）後: マイアクティビティの記録={stored} 件 / 確認待ちの残り={pending} 行 / 台帳の read={reads} 行");

    let names: Vec<String> = sqlx::query_scalar(
        "SELECT logical_source || ' / ' || display_name FROM core.source WHERE logical_source LIKE 'c03-myactivity-%'",
    )
    .fetch_all(&inbox.pool)
    .await
    .unwrap();
    println!("【4】自動登録されたマイアクティビティのソース: {names:?}");
}
