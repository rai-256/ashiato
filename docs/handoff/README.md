...
5. **上流も読む**（2026-09-18 に足した）。`python3 scripts/story_brief.py ST<NN>` の
   「**この Story に宛てられたもの**」に、この節と、別の change の `処置: deferred ST<NN>` が
   逐語で出る。**深掘りより先に読む** —— 読まずに問いを立てると同じことを二度決める。
   それまで `deferred ST<NN>` は `review_triage.py` が書式を検査するだけで、
   **宛先の Story が読む経路が無かった**（名前はあるのに受け皿が無い工程は消える）
6. **上流がまだ走っていない Story にも置いてよい**（同上）。凍結された Story だけの場所ではない ——
   宛先が読む経路ができたので、「他所で見つかった、その Story がやること」はここに集まる
