...
| 主役 | 情報密度（比例スケール 1 本） | Slider 0–100 | **77**（間隔 24px） |
...
> **「しるしを付けない」を選んだので、種類の区別は色だけになる。** 色覚に依存しない区別が要る
> 画面（稼働状況のソース別など）が出てきたら、Story の深掘りで形かラベルを足すこと。
> ここで決めたのは既定であって、禁止ではない。
...
`ui-review` subagent を独立に 2 回起動した（自分では点検していない）。全文は
`docs/reviews/ui-direction_review.md`。**第 1 回 37 件 + 第 2 回 10 件 = 47 件**。
...
| UIR-1 | 「地の深さ」の可動域が明度 4〜20% しかなく、振っても実質変わらなかった | ダーク 3〜34% / ライト 99〜66% に拡大 |
| UIR-39 / UIR-40 | UIR-1 を直した副作用で、深さを上げると「面の段数」が動かなくなっていた | 段数ぶんの余地（最小 4pt × 段数）を先に確保してから深さを配分。深さ 100 でも段差 4.0pt を保つ |
...
**指摘は 1 件も黙って捨てていない。** 60 件のうち直した 54 / 直さない 3 / 撤回して直した 3。
