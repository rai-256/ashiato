...
> |---|---|---|
> | NFR-12（手作業は Google 系のみ 2 か月に 1 回） | ST13 | Google 系の手作業に、タイムラインの書き出しと網の外に出ない手段での運搬を含む（★ 2026-09-15。Q7 / Q8）。運ぶ仕組みは作らない |
> | NFR-3（Takeout 系は最終日で見せる） | ST13 | 書庫のソースの最終日を稼働状況の画面に出す（FR-55 と同じ画面） |
> | FR-78（生存信号） | ST02 | 取り込み器そのものの論理ソース `s01-archive-inbox` に 1 日 1 回（書庫のソースには送らない。C19） |
> | FR-35 / FR-61（想定間隔・登録簿） | ST14 / ST01 | 書庫のソースを 60 日、取り込み器を 1 日で登録する。通知は作らない |

> **同じ型が ST06 からも 1 件出ている**（2026-09-18）:
>
...
>
> **訂正（2026-09-19、ST22 の specs を書いた後の独立レビュー）** —— 要件の条項の宛先を 2 つ直した:
>
> | 要件 | 上の表 / 前の訂正 | 直した先 |
> |---|---|---|
> | FR-50（削除）の**個人属性の主張**の条項 | 「消す操作は ST22」（:162。ST19 の訂正） | **ST23**。ST22 の口は滞在だけに狭めた（深掘り C3）。主張を消す操作は S-6 に足す側の Story が持つ。`docs/handoff/ST23.md`（st22 R1） |
> | FR-52（消す前に件数を出して確認） | ST23（表のまま） | **ST23 のまま。ST22 は前倒ししない**。ST22 の確認は深掘り Q4 で本人が選んだもので、対象は戻せる削除（design D13（仮）） |
>
> - **稼働状況が削除済みも数える規則（深掘り Q3。本人の決定）は `collection-coverage` の正典に書かない** ——
