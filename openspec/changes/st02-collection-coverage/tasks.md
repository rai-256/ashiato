...
- [ ] 1.4b **収集開始日を受け口の側で埋める** —— 記録または生存信号を受けたとき、
      そのソースの `collection_started_on` が `NULL` なら、その日（`Asia/Tokyo`）を書く。
...
- [ ] 3.3b `attempts`（前回の信号からの取得の試行回数）と `successes` を受け取って保存する
      ...
      検証: `cargo test -p ashiato-server heartbeat_attempt_counts` が rc=0
...
- [ ] 8.3 **格子のセルは 3 段だけ**で描く（design D10。第 5 回 Q21）——
...
- [ ] 8.5 **幅 360 px で操作対象が 24 × 24 CSS px を割らず、かつ 1 年ぶんの格子が
      横スクロールなしで収まる**ことを検査する（NFR-19 / 第 5 回 Q21）。
