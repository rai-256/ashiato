...
- [ ] 1.4b **収集開始日を受け口の側で埋める** —— 記録または生存信号を受けたとき、
      そのソースの `collection_started_on` が `NULL` なら埋める。
      **どの日付を取るか（届いた日か、最初の記録の出来事時刻か）は 11.2（本人の答え待ち）。**
      検証: `cargo test -p ashiato-server sets_started_on_first_arrival` が rc=0
...
- [ ] 6.1 `GET /coverage/achievement?from=&to=` を足す。主語の割り当てを
      ...
      **判定は「分母 × 95 % 以上」**（第 5 回 Q18。絶対値の 350 ではない）。
      達成日数と分母の両方を返す。**365 日窓の起点と終点は 11.1（本人の答え待ち）。**
...
- [ ] 8.2 1 年を週に畳んだ格子を出す。**ソースごとに格子を分け、ソース名の文字を添える**（Q15）。
      **格子の向き（週が行か列か）と実寸は 11.3（本人の答え待ち）で決まる。**
...
- [ ] 8.5 **幅 360 px で操作対象が 24 × 24 CSS px を割らないことを検査する**（NFR-19）。
      ...
      **週の帯が 24 px を満たせるかは格子の向きで決まる**（11.3 待ち）——
      横長だと週の帯は幅 6 px で割る。
...
第 5 回の反映を **2 巡目の独立レビュー**（`review/spec-r2.md`、12 件）にかけて出た 3 件。
問いは `deep-questions-r6.json`、渡す HTML は `docs/briefs/ST02-deep-r6.html`。

- [ ] 11.1 **NFR-13 の 365 日窓の起点と終点**（2 巡目 R3）。第 5 回 Q18 が閉じたのは閾値だけで、
      **どの 365 日を数えるか**が残っている。`?from=&to=` が自由なので窓の取り方で合否が変わる
- [ ] 11.2 **収集開始日にどの日付を取るか**（2 巡目 R5）。本人の逐語「届いた日」に 2 つの読みがあり、
      受信時刻を採ると**圏外で溜めて送った記録のある日が⑦「導入前」になる**
- [ ] 11.3 **格子の向き（週は行か列か）と実寸**（2 巡目 R2）。横長だと週の帯が 6 px で NFR-19 を割り、
      縦長だと縦 6,000 px を超える。**第 5 回 Q20 の選択肢の書き方が招いたもの** ——
      「行 = 週」と「GitHub の草に近い形」を並べたが、草では週は列で両立しない
