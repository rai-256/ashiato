...
- [ ] 6.1 `GET /coverage/achievement` を足す（**期間は取らない**）。主語の割り当てを
      ...
      **合否は 5 本すべての窓が閉じた日にのみ確定**し、それまでは暫定 + 残り日数を返す
      （第 7 回 Q27）。収集が始まっていないソースがあるあいだは確定日を返さない。
...
- [ ] 6.9 **確定と暫定を区別する**ことを確認する（第 7 回 Q27）——
...
- [ ] 8.5c **開いた直後に 5 ソースすべての直近 4 週が同時に見える**ことを確認する（第 7 回 Q28）——
...
- [ ] 8.6 5 ソースの達成日数と分母、5 本すべてが分母の 95 % 以上かの合否を数値で出し、
