...
- [ ] 1.4b **収集開始日を受け口の側で埋める** —— 記録または生存信号を受けたとき、
      そのソースの `collection_started_on` が `NULL` か、受けたものの出来事時刻の日より後なら、
      **その日（`Asia/Tokyo`）で更新する**。**受信時刻ではなく、記録が作られた時刻**（第 6 回 Q24）。
      **後から古い記録が届いたら開始日は前に動く**（圏外の保持がこれを起こす）。
      検証: `cargo test -p ashiato-server sets_started_on_first_arrival started_on_moves_back` が rc=0
- [ ] 1.4c **1 件も届いていないソースが「導入前」のままで「途絶」にならない**ことを確認する
...
- [ ] 3.3b `attempts`（前回の信号からの取得の試行回数）と `successes` を受け取って保存する
...
- [ ] 5.1 `GET /coverage?from=&to=` を足す。決定順序は design D7 のとおり
...
- [ ] 5.1b **停止・破棄が記録より優先されることを確認する**（第 5 回 Q19）——
...
- [ ] 6.1 `GET /coverage/achievement` を足す（**期間は取らない**）。主語の割り当てを
...
- [ ] 6.4b **分母から抜けた日は達成日にも数えない**ことを確認する ——
...
- [ ] 6.7 **窓がソースごとに別の日に始まる**ことを確認する（第 6 回 Q23）——
...
- [ ] 6.8 **365 日が経つ前でも途中経過が出る**ことを確認する（第 6 回 Q23）——
      開始日から 200 日なら分母 200 日で同じ式。
...
- [ ] 8.2 1 年を週に畳んだ格子を出す。**ソースごとに格子を分け、ソース名の文字を添える**（Q15）。
      **格子は縦長**（1 行 = 1 週、上から下へ 53 週、**新しい週が上**。第 6 回 Q25）。
      検証: `cd web && npx tsc -b && npm run lint && npm run build` がいずれも rc=0
- [ ] 8.3 **格子のセルは 3 段だけ**で描く（design D10。第 5 回 Q21。要件は NFR-23）——
...
- [ ] 8.4 **週を選ぶと、その 7 日ぶんが 7 状態の名前で文字で出る**（第 5 回 Q20 / Q21）。
      **格子で「それ以外」に畳まれた日も、ここでどの状態だったかが分かる。**
      セルにイベントハンドラを付けない（Q16）。
      検証: `cd web && npm run test -- week-select` が rc=0
- [ ] 8.5 **幅 360 px で操作対象が 24 × 24 CSS px を割らず、格子が横スクロールなしで
      収まることを検査する**（NFR-19）。**週の帯の高さが 24 px 以上**であること、
      セルが操作対象になっていないことも同じ検査で確認する。
      検証: `cd web && npm run test -- target-size` が rc=0
- [ ] 8.5b **直近の週が一番上にある**ことを確認する（第 6 回 Q25）——
      完了の判定「1 か月放置した後に開くと欠けた日が一目で分かる」を
      **スクロールなしで**満たすため（1 か月 = 4〜5 行）。
      検証: `cd web && npm run test -- newest-week-first` が rc=0
- [ ] 8.6 5 ソースの達成日数と分母、5 本すべてが分母の 95 % 以上かの合否を数値で出す
      （NFR-13 / 第 4 回 Q9 / 第 5 回 Q18）。
      検証: `cd web && npm run test -- achievement-panel` が rc=0
- [ ] 8.7 行頭のソース名と、週を選んだときに出る状態名の文字が **4.5:1 以上**であることを
...
## 12. 未決（**人間の答えを待つ。他は先に進める**）

**3 巡目の独立レビュー**（`review/spec-r3.md`、9 件）が出した 3 件。
6 件は残骸・形の問題で直した。この 3 件は**第 6 回の答えがそれぞれ生んだ新しい振る舞い**。
問いは `deep-questions-r7.json`、HTML は `docs/briefs/ST02-deep-r7.html`。

- [ ] 12.1 **収集開始日が後から前に動くのを許すか**（3 巡目 R2。Q24 が生んだ）——
      圏外で溜めた古い記録が届くと開始日が遡り、**NFR-13 の分母と窓の起点も遡る**。
      specs は別の場所で「判定式は計測開始後に変えられない」と書いており、非対称が未説明
- [ ] 12.2 **成功条件 1 の合否がいつ確定するか**（3 巡目 R3。Q23 が生んだ）——
      窓がソースごとなので 5 本の閉じる日が別々。いまのままだと
      **収集開始 10 日目のソースが 100 % で「達成」に見える**
- [ ] 12.3 **縦長の格子で 2 本目以降のソースをどう見せるか**（3 巡目 R4。Q25 が生んだ）——
      5 ソースで縦 6,500 px。「新しい週を上」は **1 本目にしか効かない**
