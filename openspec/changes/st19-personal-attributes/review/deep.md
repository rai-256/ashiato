...
- 処置: escalated — 問いにはしない（畳まない側は何も失わず費用が小さい = 扉を開けたままにする既定）。deep.md の C2 に衝突として明記し「要件へ戻すもの」に FR-22 を挙げた。本人には C の一覧で見せ、異論は番号で受ける
...
- 処置: escalated — 問いにはしない（Q1 の選択肢の説明の抜けで、読み替えは扉を開けたままにする側）。Q1 の context (e) に FR-50 の帰結と、主張については識別子で判定する読み替えを足した。deep.md の「要件へ戻すもの」に FR-50 を挙げた
...
- 処置: fixed deep.md — C12（内容の鍵に主張ごとの識別子を混ぜ、消去の後に残る鍵から値を当てられなくする）を足し、Q1 選択肢 1 の不可逆の記述を C12 に結んだ
...
- 処置: fixed deep.md — C2 に「主張ごとの識別子で中身が違えば 400 で断り、FR-22 の更新の経路に乗せない」を足した
