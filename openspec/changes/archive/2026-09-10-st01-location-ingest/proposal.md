...
- 既存の `/ingest` に**不足している 3 点を足す**:
  - **NFC 正規化（FR-27）が実装されていない** —— 処理が無い。混在させて溜めると後から直せない。
    **対象は解析済みの内容だけで、原文には手を加えない**（深掘りで本人が決定。FR-18 を優先）
...
- **取り込み口をまとめ送りに対応させる（BREAKING）** —— 深掘りで「5 分ぶんをまとめて送る」と
  決めたため、要求が複数件の配列になり、応答も 1 件ごとの結果の配列になる

## 深掘りで本人が決めたこと（2026-09-08）

上流の `deep` を飛ばしていたため、当初は 8 件の技術判断を実装者が独断で決めていた。
そのうち**要件の矛盾を解いていたもの・不可逆なもの・日常に影響するもの**を本人に戻した。

| 論点 | 決定 | 効果 |
|---|---|---|
| **原文を NFC 正規化するか**（FR-18 と扉 #16 の衝突） | **しない。`payload` だけ** | 当初案を**覆した**。specs / design / tasks を修正 |
| 水平精度が悪い記録を捨てるか | **捨てない** | 収集側でふるい落とさない（design D11） |
| 前景サービスの常時通知 | **受け入れる** | FR-1 の 60 秒間隔をそのまま実装 |
| 位置ログの既定の感度（PERM-3 と PERM-6 の衝突） | **PERM-3 どおり外部 AI 可** | 成功条件 2 を優先。要件の修正は不要 |
| 端末識別子（入れ直しで別端末になる） | **受け入れる** | 自前採番のまま |
| 送信のまとめ方 | **5 分ぶんをまとめて送る** | `/ingest` の形が変わる（上記 BREAKING） |
...
- `collection-coverage`: 収集が動いていたかの日ごとの記録。**ST01 は稼働記録を必ず書く**ので、
  件数の数え方（design D13）の置き場がここに要る。通知・停止の検知は ST02 / ST14 / ST15 が足す

> **`docs/stories/INDEX.md` の capability 表は ST01 を `record-envelope` にだけ割り当てていた。**
> `collection-coverage` も同じ理由で前倒す —— ST01 は取り込みと同じ関門で稼働記録を書くので、
> 件数の数え方（D13）が ST02 まで置き場を持たないことになる。名前は表のまま。
