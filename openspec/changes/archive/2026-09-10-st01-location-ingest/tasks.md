...
> **2026-09-10、実データで 4 項目中 3 つを確認した**（記録 16 件 / 2 端末時刻 00:58〜00:21）。
>
> | 完了の判定 | 実測 |
> |---|---|
> | 記録が時刻順に並ぶ | ✅ 16/16 |
> | 1 行を開くと欄がすべて埋まっている | ✅ 欠け 0。原文は `text` で素通し、感度は 1（PERM-3 / D10） |
> | 登録簿の 1 行だけで受け付ける | ✅ |
> | 資格情報の無い要求が 401 | ✅ smoke が毎回 |
>
> NFR-1: 生成 → 格納の最大 **740 秒**（上限 3600 秒）。5 分ごとの一括送信の形どおり。
> 重複 0。水平精度 100 m の記録も残っている（D11 が効いている）。
>
> **残っているのは「外を歩く」そのもの。** 移動幅が 36 m × 29 m しかなく、屋内で静止した
> 状態の確認にとどまる。**あわせて見るべき欠損が 1 件出ている**:
>
> - 60 秒間隔が **1 回だけ 465 秒に伸びた**（00:11:41 → 00:20:49）。ほかは全部 61 秒。
>   前景サービスはこれを防ぐためにある（D7）ので、**成功条件 1 に直接効く**。
>   端末側の電池最適化（AQUOS の省電力）か、屋内で測位できなかったかの切り分けが要る。
>   散歩のあいだに再発するか、`core.coverage` の件数が分あたり 1 件で推移するかを見る。

> **0003 を当てたあとにやること**（深掘り 第 3 回 R24 で確定）。本人の答えは
> 「まだ実データは入っていない」なので、先に歩くとその日ぶんが移行の対象になってしまう。
> merge → 配備（起動時に 0001〜0004 が当たる）→ 歩く、の順。
...
- [x] 10.15 **実機へ渡す前に塞いだ 2 件**（8.4 の準備で発覚）。
      (a) `tools/seed.sh` が `raw` を JSON の値のまま送っていた —— 9.4 の追従漏れで、
      **CI がこの台本を走らせないので誰も気付かなかった**。
      (b) 平文 HTTP が Android の既定で遮断される（design D26）。
      検証: `./tools/seed.sh normal` が `9 件を入れた` を出し、原文が text で素通しする。
      APK の `res/xml/network_security_config.xml` に接続先 1 ホストだけが載る（aapt2 で確認）

- [x] 10.16 **実機で初回起動が通らなかったのを直す**（design D27）。
      背景の位置は Android 11 以降ダイアログで取れず結果が必ず拒否で返るのに、
      それを信じて `finish()` していた —— 前景を許可した直後に必ず終了し、
      サービスが一度も起動しなかった。**テストはあったが、結果コードを直接渡していたので
      実機の挙動を写していなかった**。実状態を見る作りに変え、`onResume` からも見直す。
      検証: `MainActivityTest` 5 本。**旧実装に戻すと「背景を断られても収集は始まる」が落ちる**
