...
- [x] 5.4 契機: 起動時に前回の成功から 24 時間以上なら取得、動作中は前回の成功から 24 時間、失敗は成功を進めず 1 分後に試し直す（design D3。**仮は 1 分だけ**）。
  注入した時計で確かめる。
...
- [x] 5.5 **成功とみなすのは未送信に積み終えて帳面を書いた後**。取り込み口を止めた状態で取得し、戻した後に格納されていることを取り込み口まで通して見る（R16）。
...
- [x] 6.3 「消えた」記録を送った後も元の訪問の行が変わらないことを、取り込み口まで通して見る。
