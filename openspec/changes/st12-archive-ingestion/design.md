...
| D14 | 移行は 1 本 | —— | —— |
| D16 | 形の確認の印（Takeout の書庫の中身） | 第 2 回 Q10（本人）/ spec-r2 R4 / R5 / R10 | 知らない製品の扱いは第 3 回 Q12 で確かめている |
...
- **形**（shape）: Takeout の書庫の中の、読む対象のファイルごとに **振り分けを決めるものだけ** —— `(見分けた種類, マイアクティビティなら products の値の集合)`（spec R5 で狭めた）。
...
- **知らない製品の名前が出たときの扱いは第 3 回 Q12 で本人に確かめている**（spec R4。本人が選んだ選択肢の文は「最初に 1 回」で、再び確認待ちにするのは読み取りの拡張だった）。
  いまの specs は「そのファイルだけまた確認待ち」で書いてあり、答えで変わる（tasks 7b は答えが入るまで着手しない）
