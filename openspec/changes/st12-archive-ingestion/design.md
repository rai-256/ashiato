...
| D10 | 取り込み器の生存信号は取り込み器の論理ソース 1 本に | C17 → C19 / deep R4 / spec R8 / R9 | 仮 |
...
- `attempts` は前回の信号からの走査の回数、`successes` は 2 つの置き場をどちらも読めた走査の回数。**回数は読み手のメモリに持ち、再起動で 0 から数え直す**（再起動の後の最初の信号は回数が少なく出る。仮。
  反転条件: 回数の欠けが取得率の読みを誤らせると分かったとき、`archive_sighting` と同じ表に回数を持つ）
...
- 書庫のソースの格子の群の頭（Must の 5 本の直後）に「直近に置いた書庫」の箱: `<file_name>`・`<見つけた時刻>`・「入った N · 既にあった N · 読めなかった N」。
  ...
  `inbox.capturable = false` なら「<置き場> が読めません」を足す
