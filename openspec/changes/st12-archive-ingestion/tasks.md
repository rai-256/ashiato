...
- **D1 / D2 のマイアクティビティの名前 / D3 / D5 の大きなファイルの読み方 / D6 / D7 の場所の上限と失敗の回数 / D9 の写しの置き場と補足の読み取り / D10 / D13 は（仮）決め。** 反転条件は `design.md`。変えたらその D 番号を書き直す
- **第 2 回の深掘り（Q9 / Q10 / Q11）の答えで変わる箇所**: 箱の位置と読んでいる間の表示（10.3）/ マイアクティビティの名前（5.5）/ 写しの設定の意味（7.2）。**答えが `deep.md` に入るまで、この 3 つのタスクに着手しない**
...
- [ ] 3.3 `run()` から取り込み器を `tokio::spawn` で起こす（走査と読み手の 2 つ。読み手は 1 冊ずつ、読んでいる間も走査は続く。design D1）。
...
- [ ] 6.2 1 件が読めなくても続け、読めなかった件数と場所（先頭 100 件）を残す。DB の失敗で中断したら通常の台帳の行を書かない。続けて 3 回中断したら `store_failed` の行を 1 つ書き、以後 1 時間に 1 回読み直す。
...
- [ ] 7.2 写し（中身の名前・同じ中身は 1 つ・読んだ製品のファイルだけ・設定が `false` なら作らない）と `core.archive_file` の目録。**第 2 回 Q11 の答えが `deep.md` に入ってから着手**（設定の意味が変われば Scenario も変わる）。
...
- [ ] 9.1 `GET /archives/status`（`sources[].last_event_on` と `last_archive_created_at`、`latest_archive`（`previously_read_at` を含む）、`inbox`）を足し、OpenAPI に載せる。最終日は台帳の `max_event_at` から導く（design D11）。
...
- [ ] 10.4 ひとスクロールと 360 px。
  ...
  検証: `VT archive-one-scroll.test.tsx`（書庫のソース 12 本（固定の 10 本 + マイアクティビティ 2 本）と箱を足した応答で Must の最後の格子の下端 ≤ 1,280 px、横スクロール無し、週の帯 ≥ 24 px）、
...
- [ ] 12.4 PR 本文に **仮決め（D1 / D2 / D3 / D5 / D6 / D7 / D9 / D10 / D13）と反転条件**、**写しをバックアップ（ST30）に入れる申し送り**（design D9）を列挙する。
  検証: `body=$(gh pr view --json body -q .body); for d in 1 2 3 5 6 7 9 10 13; do grep -q "D${d}（仮）" <<<"$body" || { echo "D${d} が無い"; exit 1; }; done; grep -q ST30 <<<"$body"` rc=0
