...
- [ ] 1.1 `migrations/0007_source_columns.sql` で `core.source` に `external_id_kind text NOT NULL DEFAULT 'record' CHECK (external_id_kind IN ('record','subject','none'))`、`retired_on date`、`succeeds text REFERENCES core.source(logical_source)` を足す。検証: `psql -c "\d core.source"` に 3 列が出て、`cargo test --test migrations` が rc=0（移行を当て直しても表が壊れない）
